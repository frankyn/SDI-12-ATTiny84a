...
#define PIN            PB1  //PB0
#define DPIN           DDB1 //DDRB0
#define DIRECTION_PORT DDRB //DDRB
#define IO_PORT        PORTB //PORTB
#define PIN_STATUS     PINB //PINB
#define PIN_STATUS_PIN PORTB1
#define PCMSK          PCMSK1
#define PCMSKPIN       PCINT9
#define GIMSKPIN       PCIE1
...
void SDI12::setState(uint8_t state){
  if(state == HOLDING){
    // Set direction of pin in pure avr as OUTPUT
    DIRECTION_PORT = (1<<DPIN);
    ...
    //*digitalPinToPCMSK(PB0) &= ~(1<<digitalPinToPCMSKbit(PB0));
    PCMSK &= ~(1<<PCMSKPIN);
    return; 
  }
  if(state == TRANSMITTING){
    // Set direction of pin in pure avr
    //pinMode(_dataPin,OUTPUT);
    DIRECTION_PORT = (1<<DPIN);
  ...
  if(state == LISTENING) {
    ...
    // Set direction of pin in pure avr as INPUT
    DIRECTION_PORT = (0<<DPIN);
    
    //interrupts();				// supplied by Arduino.h, same as sei()
    sei();
    
    //*digitalPinToPCICR(PB0) |= (1<<digitalPinToPCICRbit(PB0));
    //*digitalPinToPCMSK(PB0) |= (1<<digitalPinToPCMSKbit(PB0));
    GIMSK |= (1<<GIMSKPIN);    //
    PCMSK |= (1<<PCMSKPIN);   // Pin Change Enable Mask PCINT5 (Here)
...
void SDI12::sendCommand(char * cmd){
...
{
  if (PIN_STATUS&(1<<PORTA5))				// 6.2.1 - Start bit?
